...
class IndexedDBTransactionManager {
  ...
  Future<DbResult<T>> executeReadOperation<T>(
    ...
    try {
      final transaction = _database.transaction([_storeName].map((e) => e.toJS).toList().toJS, 'readonly');
  ...
  Future<DbResult<T>> executeWriteOperation<T>(
    ...
    try {
      final transaction = _database.transaction([_storeName].map((e) => e.toJS).toList().toJS, 'readwrite');
      final store = transaction.objectStore(_storeName);
  ...
  Future<DbResult<T>> createRequestOperation<T>(
    ...
    request.onsuccess = ((web.Event event) {
      try {
        ...
        completer.complete(Err(DbError.databaseError('$operation parsing failed: $e')));
      }
    }).toJS;
    ...
    request.onerror = ((web.Event event) {
      ...
      completer.complete(Err(DbError.databaseError('$operation failed: $error')));
    }).toJS;
